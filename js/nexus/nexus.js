...
var maxPending    = 3;
var maxBlocked    = 3;
var maxCacheSize  = 256*(1<<30); //TODO DEBUG
var drawBudget    = 1*(1<<30);
...
Mesh.prototype = {
	open: function(url) {
		var mesh = this;
		mesh._url = url;
		mesh.httpRequest(0, 92, function() {
	...
	httpRequest: function(start, end, load, error, abort, type) {
		...
		// get data from file servlet
		r.open('POST', './partialFileServlet.php', true);
		r.responseType = type;
		r.setRequestHeader("Range", "bytes=" + start + "-" + (end -1));
		r.setRequestHeader("Model", "model=" + this.url());
		r.onload = function(){
			switch (this.status){
				case 206:
                			load.bind(this)();
	...
	importHeader: function(view) {
		...
		if(magic != 0x4D504820) return null; //0x4E787320
		var sCount = getUint32(view);
		var h = {};
		h.version = getUint32(view);
		h.verticesCount = getUint64(view);
		h.facesCount = getUint64(view);
		h.signature = this.importSignature(view);
		h.nodesCount = getUint32(view) - 32;
		h.patchesCount = getUint32(view) - 32;
...
return { Mesh: Mesh, Renderer: Instance, Renderable: Instance, Instance:Instance,
	...
	setTargetError:setTargetError, setTargetFps:setTargetFps, setMaxCacheSize:setMaxCacheSize, setDrawBudget:setDrawBudget };
